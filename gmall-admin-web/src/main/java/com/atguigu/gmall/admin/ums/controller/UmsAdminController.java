...
/**
 * 后台用户管理
 * SpringMVC支持使用 【JSR303(这是标准 规范)】 方式进行校验
...
//        if(errorCount>0){
//            List<FieldError> fieldErrors = result.getFieldErrors();//获取所有的操作
//            fieldErrors.forEach((fieldError)->{ //然后打印
...
//        }
    ...
    public Object login(@RequestBody UmsAdminLoginParam umsAdminLoginParam, BindingResult result) {
        //@RequestBody == 将请求的数据封装成==》 UmsAdminLoginParam
        //去数据库登陆
